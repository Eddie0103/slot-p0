...
2. 把下面「──── 提示詞開始 ────」到「──── 提示詞結束 ────」之間的內容整段貼進去
3. 每次檢查前，只需要更新「本次檢查重點」那一段
...
| 日期 | 範圍 | 審查者 | 結果 |
|---|---|---|---|
| 2026-08-02 | P1 任務 1（資料模型、migration、禁止箭頭） | Fable 5 | 待執行 |
...
3. **判斷有分歧的地方**（你的看法與前一位工程師不同，但兩邊都說得通）

每一項請寫清楚：在哪個檔案哪一行、為什麼是問題、你建議怎麼處理。

沒發現問題的部分也請明說「這部分我檢查過，沒有發現問題」，不要只列問題讓人無法判斷你看了多少。
